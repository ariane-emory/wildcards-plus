...
const raw                 = String.raw;
const syntax_chars        = '@#$%';
const structure_chars     = '{}|'; // NOT including brackets here
const comment_beginning   = raw`\/\/|\/\*`;

const plain_text_head = (extra = '') =>
      raw`(?:` +
      raw`(?:[()\[\]]*)` +
      raw`(?![${syntax_chars}]|${comment_beginning})` +
      raw`(?:\\.|(?![\s${structure_chars}${extra}]|${comment_beginning})\S)` +
      raw`)`;
const plain_text_tail = (extra = '') =>
      raw`(?:\\.|(?![\s${structure_chars}${extra}]|${comment_beginning})\S)`;

const plain_text          = r_raw`${plain_text_head('' )}${plain_text_tail('')}*`;
const plain_text_no_semis = r_raw`${plain_text_head(';')}${plain_text_tail(';')}*`;
